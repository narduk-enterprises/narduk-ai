...
```bash
git clone https://github.com/narduk-enterprises/narduk-ai.git my-project
cd my-project
pnpm install
...
| --------------- | ---------- | -------------- |
| `narduk-ai`     | `dev`      | Shared secrets |
| `narduk-ai`     | `dev_auth` | `example-auth` |

**First-time setup for example-auth:**

```bash
cd apps/example-auth
doppler setup --project narduk-ai --config dev_auth --no-interactive
```
